...
InputState readUserInput() {
    ...
    if (currentCounter != lastCounter) {
        ...
        delay(100); // Simple debounce for rotary input, adjust as needed
    }
...
void interfaceLoop(const InputState& input) {
    ...
    if ((!editingAlarm && !editingMinute) && (!changeWeather) && (input.rotaryDirection == 1 || input.rotaryDirection == -1 
        ...
        delay(100); // Debounce delay for rotary input, adjust as needed
    ...
    switch (currentMenu) {
        ...
        case MENU_CLOCK: {
            ...
            showAlarmStatus();     // ✅ Overlay alarm icon/status
